...
function personPicker(name, label, selectedIds = [], { multiple = false, help = '', className = '', searchPlaceholder = 'Buscar pessoa pelo nome' } = {}) {
  const selected = selectedIds.map(peopleById).filter(Boolean);
  const options = data.people.map((person) => {
    const isSelected = selectedIds.includes(person.id);
    const searchable = [person.fullName, person.whatsapp, person.email].filter(Boolean).join(' ').toLowerCase();
    return `<button type="button" class="person-picker-option ${isSelected ? 'selected' : ''}" data-picker-select="${name}" data-person-id="${person.id}" data-picker-multiple="${multiple}" data-picker-search-value="${h(searchable)}" aria-pressed="${isSelected}">${personAvatar(person, 'small')}<span class="person-picker-copy"><strong>${h(person.fullName)}</strong><small>${h(person.whatsapp)}</small></span><span class="person-picker-check" aria-hidden="true">${isSelected ? '✓' : ''}</span></button>`;
  }).join('') || '<p class="empty-state">Cadastre uma pessoa antes de vinculá-la.</p>';
  return `<section class="person-picker ${h(className)}" data-person-picker="${name}"><label>${h(label)}<input type="search" data-picker-search="${name}" placeholder="${h(searchPlaceholder)}" autocomplete="off"></label><input type="hidden" name="${name}" value="${h(selectedIds.join(','))}"><div class="person-picker-selected" data-picker-selected="${name}">${selected.map((person) => `<span>${h(person.fullName)}<button type="button" aria-label="Remover ${h(person.fullName)}" data-picker-remove="${name}" data-person-id="${person.id}">×</button></span>`).join('') || '<small>Nenhuma pessoa selecionada.</small>'}</div><div class="person-picker-options" data-picker-options="${name}">${options}</div>${help ? `<p class="picker-help">${h(help)}</p>` : ''}</section>`;
}

function ministryIcon(name) {
  return ({ 'Família': '⌂', Jovens: '✦', Kids: '◌', Mulheres: '◐' })[name] || '◈';
}

function ministryPicker(name, selectedId, label = 'Ministério', { className = '', showIcons = false } = {}) {
  const choices = ministries().map((ministry) => {
    const isSelected = selectedId === ministry.id;
    return `<button type="button" class="choice-card ${isSelected ? 'selected' : ''}" data-ministry-select="${name}" data-ministry-id="${ministry.id}" aria-pressed="${isSelected}">${showIcons ? `<span class="choice-card-icon" aria-hidden="true">${ministryIcon(ministry.name)}</span>` : ''}<span>${h(ministry.name)}</span>${showIcons ? `<span class="choice-card-check" aria-hidden="true">${isSelected ? '✓' : ''}</span>` : ''}</button>`;
  }).join('') || '<p class="empty-state">Nenhum ministério disponível. A migration organizacional precisa ser aplicada.</p>';
  return `<section class="choice-picker choice-picker--ministry ${h(className)}"><label>${h(label)}</label><input type="hidden" name="${name}" value="${h(selectedId || '')}"><div class="choice-grid" data-ministry-options="${name}" role="group" aria-label="${h(label)}">${choices}</div></section>`;
}

function weekdayPicker(selectedDay) {
  const days = ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'];
  return `<section class="choice-picker choice-picker--weekday"><label>Dia da semana</label><input type="hidden" name="weekday" value="${h(selectedDay ?? '')}"><div class="weekday-grid" role="group" aria-label="Dia da semana">${days.map((day, index) => { const isSelected = Number(selectedDay) === index; return `<button type="button" class="weekday-choice ${isSelected ? 'selected' : ''}" data-weekday-select="${index}" aria-pressed="${isSelected}">${day}</button>`; }).join('')}</div></section>`;
}
...
function ministryForm() {
  return `<form id="ministry-form" class="structured-form ministry-form"><p class="form-help">Cadastre um ministério para organizar suas células e equipes. Nenhuma pessoa ou célula será criada automaticamente.</p><label>Nome do ministério<input name="name" maxlength="100" required placeholder="Ex.: Homens"></label><div class="form-actions"><button type="button" class="button outline" data-close-modal>Cancelar</button><button class="button primary">Salvar ministério</button></div></form>`;
}

function supervisorForm() {
  return `<form id="supervisor-form" class="admin-form supervisor-form"><p class="admin-form-intro">Selecione o ministério e a pessoa que será vinculada como supervisor.</p><section class="admin-form-section"><div class="admin-form-section-heading"><p class="eyebrow">ETAPA 1</p><h3>Ministério</h3></div>${ministryPicker('ministryId', '', 'Escolha o ministério', { className: 'admin-ministry-picker', showIcons: true })}</section><section class="admin-form-section"><div class="admin-form-section-heading"><p class="eyebrow">ETAPA 2</p><h3>Pessoa</h3></div>${personPicker('personId', 'Busque uma pessoa cadastrada', [], { className: 'admin-person-picker', searchPlaceholder: 'Buscar pessoa pelo nome, telefone ou e-mail...', help: 'Se a pessoa ainda não existir, cadastre-a em Membros e volte para vinculá-la.' })}</section><section class="admin-selection-summary" data-supervisor-summary hidden></section><div class="admin-modal-footer"><button type="button" class="button outline" data-close-modal>Cancelar</button><button class="button primary">Vincular Supervisor</button></div></form>`;
}
...
function modalView() {
  ...
  return `<div class="modal-backdrop"><section class="modal ${modal.kind === 'supervisor-form' ? 'admin-modal' : ''}" role="dialog" aria-modal="true" aria-labelledby="modal-title" tabindex="-1"><div class="modal-header"><div><p class="eyebrow">CEM CONNECT</p><h2 id="modal-title">${title}</h2></div><button class="modal-close" data-close-modal aria-label="Fechar janela">${icons.close}</button></div>${body}</section></div>`;
}
...
function bindEvents() {
  ...
  document.querySelectorAll('[data-picker-search]').forEach((input) => input.addEventListener('input', () => { const term = input.value.toLowerCase().trim(); document.querySelectorAll(`[data-picker-options="${input.dataset.pickerSearch}"] .person-picker-option`).forEach((option) => { option.hidden = !String(option.dataset.pickerSearchValue || option.innerText).includes(term); }); }));
...
function syncSupervisorSelectionSummary() { const summary = document.querySelector('[data-supervisor-summary]'); if (!summary) return; const ministryId = document.querySelector('input[name="ministryId"]')?.value; const personId = document.querySelector('input[name="personId"]')?.value; const ministry = ministries().find((item) => item.id === ministryId); const person = peopleById(personId); if (!ministry || !person) { summary.hidden = true; summary.innerHTML = ''; return; } summary.hidden = false; summary.innerHTML = `<p class="eyebrow">CONFIRMAÇÃO</p><h3>Vínculo a ser criado</h3><dl><div><dt>Ministério</dt><dd><span class="summary-ministry-icon" aria-hidden="true">${ministryIcon(ministry.name)}</span>${h(ministry.name)}</dd></div><div><dt>Pessoa</dt><dd>${personAvatar(person, 'small')}<span><strong>${h(person.fullName)}</strong><small>${h(person.whatsapp)}</small></span></dd></div></dl>`; }
function renderPersonPickerSelection(name, ids) { const target = document.querySelector(`[data-picker-selected="${name}"]`); const input = document.querySelector(`input[name="${name}"]`); if (!target || !input) return; input.value = ids.join(','); target.innerHTML = ids.map(peopleById).filter(Boolean).map((person) => `<span>${h(person.fullName)}<button type="button" aria-label="Remover ${h(person.fullName)}" data-picker-remove="${name}" data-person-id="${person.id}">×</button></span>`).join('') || '<small>Nenhuma pessoa selecionada.</small>'; document.querySelectorAll(`[data-picker-options="${name}"] [data-picker-select]`).forEach((button) => { const isSelected = ids.includes(button.dataset.personId); button.classList.toggle('selected', isSelected); button.setAttribute('aria-pressed', String(isSelected)); button.querySelector('.person-picker-check').textContent = isSelected ? '✓' : ''; }); target.querySelectorAll('[data-picker-remove]').forEach((button) => button.addEventListener('click', () => removePersonPicker(name, button.dataset.personId))); syncSupervisorSelectionSummary(); }
function updatePersonPicker(name, personId, multiple) { const current = pickerIds(name); const ids = multiple ? (current.includes(personId) ? current.filter((id) => id !== personId) : [...current, personId]) : [personId]; renderPersonPickerSelection(name, ids); }
function removePersonPicker(name, personId) { renderPersonPickerSelection(name, pickerIds(name).filter((id) => id !== personId)); }
function updateMinistryPicker(name, ministryId) { const input = document.querySelector(`input[name="${name}"]`); if (!input) return; input.value = ministryId; document.querySelectorAll(`[data-ministry-options="${name}"] [data-ministry-select]`).forEach((button) => { const isSelected = button.dataset.ministryId === ministryId; button.classList.toggle('selected', isSelected); button.setAttribute('aria-pressed', String(isSelected)); button.querySelector('.choice-card-check')?.replaceChildren(isSelected ? '✓' : ''); }); syncSupervisorSelectionSummary(); }
function updateWeekdayPicker(day) { const input = document.querySelector('input[name="weekday"]'); if (!input) return; input.value = day; document.querySelectorAll('[data-weekday-select]').forEach((button) => { const isSelected = button.dataset.weekdaySelect === day; button.classList.toggle('selected', isSelected); button.setAttribute('aria-pressed', String(isSelected)); }); }
