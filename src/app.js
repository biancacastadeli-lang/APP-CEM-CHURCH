...
function personPicker(name, label, selectedIds = [], { multiple = false, help = '' } = {}) {
  ...
  const options = data.people.map((person) => {
    ...
    return `<button type="button" class="person-picker-option ${isSelected ? 'selected' : ''}" data-picker-select="${name}" data-person-id="${person.id}" data-picker-multiple="${multiple}" aria-pressed="${isSelected}">${personAvatar(person, 'small')}<span class="person-picker-copy"><strong>${h(person.fullName)}</strong><small>${h(person.whatsapp)}</small></span><span class="person-picker-check" aria-hidden="true">${isSelected ? '✓' : ''}</span></button>`;
...
function ministryPicker(name, selectedId, label = 'Ministério') {
  const choices = ministries().map((ministry) => {
    ...
    return `<button type="button" class="choice-card ${isSelected ? 'selected' : ''}" data-ministry-select="${name}" data-ministry-id="${ministry.id}" aria-pressed="${isSelected}">${h(ministry.name)}</button>`;
  }).join('') || '<p class="empty-state">Nenhum ministério disponível. A migration organizacional precisa ser aplicada.</p>';
  return `<section class="choice-picker choice-picker--ministry"><label>${h(label)}</label><input type="hidden" name="${name}" value="${h(selectedId || '')}"><div class="choice-grid" data-ministry-options="${name}" role="group" aria-label="${h(label)}">${choices}</div></section>`;
}
