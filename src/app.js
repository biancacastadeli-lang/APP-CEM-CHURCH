...
function cellForm(cell = null) {
  ...
  return `<form id="cell-form"><input type="hidden" name="id" value="${value('id')}"><section class="form-section"><p class="eyebrow">IDENTIFICAÇÃO</p><label>Nome da célula<input name="name" value="${value('name')}" required></label>${ministryPicker('ministryId', ministryId)}${weekdayPicker(item.weekday)}<label>Horário<input name="meetingTime" type="time" value="${h(String(item.meetingTime || '').slice(0,5))}" required></label></section><section class="form-section"><p class="eyebrow">LOCAL</p><label>Local/endereço<input name="addressLine" value="${value('addressLine')}" required></label><label>Bairro<input name="neighborhood" value="${value('neighborhood')}" required></label><label>Cidade<input name="city" value="${value('city')}" required></label><label>Estado<input name="state" maxlength="2" value="${value('state')}" required></label></section><section class="form-section"><p class="eyebrow">ESTRUTURA</p>${personPicker('supervisorPersonId', 'Supervisor', supervisorId ? [supervisorId] : [], { help: 'A pessoa precisa estar vinculada como Supervisora ao mesmo ministério.' })}${personPicker('leaderIds', 'Lideranças', leaderIds, { multiple: true, help: 'Uma célula pode possuir mais de uma liderança. A credencial de acesso continua separada deste vínculo.' })}<label class="consent"><input name="isActive" type="checkbox" ${item.isActive === false ? '' : 'checked'}> Célula ativa</label></section><div class="form-actions"><button type="button" class="button outline" data-close-modal>Cancelar</button><button class="button primary">Salvar célula</button></div></form>`;
}
...
function adminBrandingForm() { const persisted = data.brandingAdministration?.church; const logo = persisted?.logo_url || ''; return `<form id="admin-branding-form"><p class="form-help">Envie PNG, JPEG ou WebP de até 5 MB. O arquivo fica no Storage seguro da igreja.</p><label>Nome do aplicativo<input name="appName" value="${h(persisted?.app_name || '')}" placeholder="CEM CONNECT" required></label><label>Nome da igreja<input name="churchName" value="${h(persisted?.church_name || '')}" placeholder="CEM Church" required></label><section class="asset-control"><p>Logotipo atual</p>${logo ? `<img src="${h(logo)}" alt="Pré-visualização do logotipo atual">` : '<div class="asset-placeholder" aria-hidden="true">C</div>'}<label>Substituir logotipo<input name="logoFile" type="file" accept="image/png,image/jpeg,image/webp"></label><label class="consent"><input name="removeLogo" type="checkbox"> Remover e restaurar o visual padrão</label></section><div class="form-actions"><button type="button" class="button outline" data-close-modal>Cancelar</button><button class="button primary">Salvar identidade</button></div></form>`; }
...
async function saveAdminBranding(event){event.preventDefault();const form=new FormData(event.currentTarget);const body={appName:form.get('appName'),churchName:form.get('churchName')};const file=form.get('logoFile');try{if(file instanceof File&&file.size)body.logoAsset=await uploadBrandingAsset('logo',file);else if(form.has('removeLogo'))body.logoAsset=null;await api('/api/admin/branding/church',{method:'PUT',body:JSON.stringify(body)});modal=null;await hydrateBranding();await hydrate();notify('Identidade atualizada.')}catch(e){notify(e.message)}}
